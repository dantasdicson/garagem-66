...
export default function OrcamentosPage() {
  ...
  return (
    <section className="page-section">
      <div className="page-heading"><div><p className="eyebrow">Atendimento</p><h1>Orçamentos</h1>
        <p className="lead">Consulte os serviços e peças previstos e acompanhe a decisão do cliente.</p></div></div>
      ...
      <div className={podeEditar ? "management-grid" : "management-grid single-column"}>
        <div className="budget-workspace">
          <div className="table-card">{carregando ? <p className="muted" role="status">Carregando orçamentos...</p> : (
          ...
          {selecionado ? <article className={`budget-detail budget-detail-${selecionado.status.toLowerCase()}`} aria-labelledby="orcamento-detalhes">
            ...
            {selecionado.observacoes ? <p className="budget-note"><strong>Observações:</strong> {selecionado.observacoes}</p> : null}
            <div className="budget-items"><section><h3>Serviços previstos</h3>{selecionado.servicos_previstos.length ? <ul>{selecionado.servicos_previstos.map((item) => <li key={item.id}><span><strong>{item.descricao}</strong><small>{item.quantidade} × {moeda(item.valor_unitario)}</small></span><span><strong>{moeda(item.valor_total)}</strong>{podeEditar && selecionado.status === "AGUARDANDO_APROVACAO" ? <button className="remove-action" type="button" disabled={Boolean(processando)} onClick={() => removerItem("servicos", item.id)}>Remover</button> : null}</span></li>)}</ul> : <p className="muted">Nenhum serviço previsto.</p>}</section>
              <section><h3>Peças previstas</h3>{selecionado.pecas_previstas.length ? <ul>{selecionado.pecas_previstas.map((item) => <li key={item.id}><span><strong>{item.peca_nome || `Peça #${item.peca}`}</strong><small>{item.peca_codigo ? `${item.peca_codigo} · ` : ""}{item.quantidade} × {moeda(item.valor_unitario)}</small></span><span><strong>{moeda(item.valor_total)}</strong>{podeEditar && selecionado.status === "AGUARDANDO_APROVACAO" ? <button className="remove-action" type="button" disabled={Boolean(processando)} onClick={() => removerItem("pecas", item.id)}>Remover</button> : null}</span></li>)}</ul> : <p className="muted">Nenhuma peça prevista.</p>}</section></div>
            {podeEditar && selecionado.status === "AGUARDANDO_APROVACAO" ? <div className="budget-editor"><form onSubmit={adicionarServico}><h3>Adicionar serviço</h3><label htmlFor="servico-descricao">Descrição</label><input id="servico-descricao" required value={formServico.descricao} onChange={(e) => setFormServico((valor) => ({ ...valor, descricao: e.target.value }))} /><div className="field-row"><div><label htmlFor="servico-quantidade">Quantidade</label><input id="servico-quantidade" type="number" min="1" required value={formServico.quantidade} onChange={(e) => setFormServico((valor) => ({ ...valor, quantidade: e.target.value }))} /></div><div><label htmlFor="servico-valor">Valor unitário</label><input id="servico-valor" type="number" min="0" step="0.01" required value={formServico.valor_unitario} onChange={(e) => setFormServico((valor) => ({ ...valor, valor_unitario: e.target.value }))} /></div></div><button className="button button-secondary" disabled={Boolean(processando)} type="submit">{processando === "servico" ? "Adicionando..." : "Adicionar serviço"}</button></form>
              <form onSubmit={adicionarPeca}><h3>Adicionar peça</h3><label htmlFor="orcamento-peca">Peça</label><select id="orcamento-peca" required value={formPeca.peca} onChange={(e) => { const id = e.target.value; const peca = pecasPorId.get(Number(id)); setFormPeca((valor) => ({ ...valor, peca: id, valor_unitario: peca?.valor_unitario || valor.valor_unitario })); }}><option value="">Selecione</option>{pecas.map((peca) => <option key={peca.id} value={peca.id}>{peca.codigo} — {peca.nome}</option>)}</select><div className="field-row"><div><label htmlFor="peca-quantidade">Quantidade</label><input id="peca-quantidade" type="number" min="1" required value={formPeca.quantidade} onChange={(e) => setFormPeca((valor) => ({ ...valor, quantidade: e.target.value }))} /></div><div><label htmlFor="peca-valor">Valor unitário</label><input id="peca-valor" type="number" min="0" step="0.01" required value={formPeca.valor_unitario} onChange={(e) => setFormPeca((valor) => ({ ...valor, valor_unitario: e.target.value }))} /></div></div><button className="button button-secondary" disabled={Boolean(processando)} type="submit">{processando === "peca" ? "Adicionando..." : "Adicionar peça"}</button></form></div> : null}
